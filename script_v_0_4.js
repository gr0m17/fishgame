...
class Bubble {
  constructor() {
    ...
    this.value = 1 * this.seed;
    this.x = Math.random() * canvas.width;
    this.y = canvas.height + 100 + Math.random() * canvas.height;
    this.radius = 50 / this.seed;
    this.speed = Math.random() * 5 + this.seed;
    this.distance;
    this.counted = false;
  }
  update() {
  ...
  for (let i = 0; i < bubblesArray.length; i++) {
    ...
    if (bubblesArray[i].distance < bubblesArray[i].radius + player.radius) {
      ...
      if (bubblesArray[i].counted == false) {
        ...
        score = score + bubblesArray[i].value;
        bubblesArray.splice(i, 1);
      }
      i--;
    }
...
function setFishScore(fishScore) {
  if (fishScore !== "" && fishScore != null) {
    setCookie("fishScore", fishScore, 365);
  }
  // updateHighscore(fishScore, deviceID);
}
